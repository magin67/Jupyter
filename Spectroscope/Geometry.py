...
def vSetTri(r = 1, size = 2):
    # Треугольная решетка:
    # r - шаг, минимальное расстояние между узлами
    # size - размер стороны
    eSet = mSetRegular(3, r)
    print(eSet)
    startx, starty = eSet[0][0], eSet[0][1] 
    dx, dy = eSet[1][0] - startx, eSet[1][1] - starty
    mSet = []
    mSet.append((startx, starty))
    for i in range(1, size):
        x = startx + i*dx 
        y = starty - i*dy 
        for j in range(i+1):
            mSet.append((x, y + 2*dy*j))
    print(mSet)
    return mSet
...
def mRadialGrid(order = 3, size = [0, 2], step = 1., accur=8):
    # Координаты узлов радиальной сетки. Узлы расположены на окружностях с шагом step.
    # На расстоянии r количество узлов равно n*r
    #    order - порядок (симметрия) сетки,
    #    step - шаг приращения радиуса (минимальный радиус)
    #    size - два размера границы (количество узлов между узлами симметрии): внутренний и внешний
    mSet = []
    sizeFrom, sizeTo = max(size[0], 1), size[1]  
    for m in range(sizeFrom, sizeTo):
        fi = 2*NP.pi/float(order*m)
        cr = step*m
        for i in range(order*m):
            x = round(cr*NP.cos(fi*i), accur)
            y = round(cr*NP.sin(fi*i), accur)
            mSet.append((x, y))
    
    if size[0] == 0: mSet.append((0, 0))
    return mSet

def mPolygonGrid(order = 3, size = [0, 2], step = 1., accur=8):
    # Координаты узлов центральной сетки. Узлы расположены на сторонах многоугольника с шагом step.
    # На расстоянии r количество узлов равно n*r
    #    order - порядок (симметрия) сетки,
    #    step - шаг приращения радиуса (минимальный радиус)
    #    size - два размера границы (количество узлов на одной стороне): внутренний и внешний
    def AddLineNodes():
        # Разбивает линию между двумя точками
        dx, dy = (cTo[0] - cFrom[0])/m, (cTo[1] - cFrom[1])/m 
        for j in range(m-1):
            xd = round(cFrom[0] + (j+1)*dx, accur)   
            yd = round(cFrom[1] + (j+1)*dy, accur)   
            mSet.append((xd, yd))
